...
A GCC 16 toolchain for cross-compiling to `arm-none-eabi` targets.

This toolchain must be built on Linux, and targets `arm-none-eabi`. Run `python build-arm-none-eabi-gcc.py --host-arch i686|x86_64 --host-platform linux|mingw32` to build your
preferred toolchain.
